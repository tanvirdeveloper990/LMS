                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                           ...
                           <td class="px-5 py-3">
                                ...
                                @else
                                <span class="px-2.5 py-1 text-xs font-semibold text-gray-600 bg-gray-100 rounded-full">
                                    Pending
                                </span>
                                @endif
                            </td>
    ...
    $(document).on('click', '.viewOrderBtn', function() {
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                let coupon = parseFloat(res.coupon) || 0;
                let total = subTotal + deliveryCharge - coupon;
                let paid = parseFloat(res.paid) || 0;
                let due = total - paid;
                ...
                <div class="border-b pb-3 mb-3">
                    ...
                    <p class="text-gray-600 text-sm">Payment Status: <span class="font-medium capitalize">${res.payment_status}</span></p>
                    <p class="text-gray-600 text-sm">Order Status: <span class="font-medium capitalize">${res.status}</span></p>
                </div>
                ...
                res.order_items.forEach(item => {
                    ...
                    <tr class="border-t">
                        ...
                        <td class="border px-3 py-2 text-right">${itemSubTotal.toFixed(2)}</td>
                    </tr>
                    `;
                });
                ...
                <div class="text-right mt-4 text-gray-700 space-y-1">
                    <p><strong>Sub Total:</strong> ${subTotal.toFixed(2)}</p>
                    <p><strong>Delivery Charge:</strong> ${deliveryCharge.toFixed(2)}</p>
                    ${coupon ? `<p><strong>Coupon Discount:</strong> -${coupon.toFixed(2)}</p>` : ''}
