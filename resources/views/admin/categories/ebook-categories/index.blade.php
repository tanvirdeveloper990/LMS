...
@section('title', 'Ebook Categories List')

@section('content')
<div class="container-fluid py-4">
    <div class="card shadow-lg rounded-3">
        <div class="card-header d-flex justify-content-between align-items-center bg-gradient-purple text-white">
            <h5 class="mb-0">Ebook Categories List</h5>
            <a href="{{ route('admin.ebook-categories.create') }}" class="btn btn-light btn-sm">
                        ...
                        <tr data-id="{{ $category->id }}">
                            ...
                            <td class="text-center">
                                <div class="d-flex justify-content-center gap-2 flex-wrap">
                                    <a href="{{ route('admin.ebook-categories.edit', $category->id) }}" class="btn btn-primary btn-sm">
                                        <i class="fa fa-edit"></i>
                                    </a>
                                    <form id="delete-form-{{ $category->id }}" action="{{ route('admin.ebook-categories.destroy', $category->id) }}" method="POST" class="d-none">
                                        @csrf
                                        @method('DELETE')
                                    </form>
    ...
    if (sortableBody) {
        Sortable.create(sortableBody, {
            ...
            onEnd: function () {
                ...
                fetch('{{ route("admin.ebook-categories.updateOrder") }}', {
                    method: 'POST',
                    headers: {
