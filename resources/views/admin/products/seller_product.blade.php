...
<div class="container-fluid py-2 py-md-4">
    <div class="card shadow-lg rounded-3">
        ...
        <div class="card-header d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-2 bg-gradient-purple text-white">
            <h5 class="mb-0">Seller Products List</h5>
        </div>

        {{-- Filter Section --}}
        <div class="card-body border-bottom bg-light p-2 p-md-3">
            <form action="{{ route('admin.seller.product') }}" method="GET">
                <div class="row g-2 g-md-3">
                   {{-- Shop Name Dropdown --}}
                    <div class="col-12 col-md-6 col-lg-3">
                        <label for="vendor_id" class="form-label small fw-semibold mb-1">Shop Name</label>
                        <select name="vendor_id" id="vendor_id" class="form-select form-select-sm">
                            <option value="">All Shops</option>
                            @foreach($sales as $vendor)
                    ...
                    <div class="col-12 col-lg-12">
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <a href="{{ route('admin.seller.product') }}" class="btn btn-secondary btn-sm">
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead class="table-light text-uppercase small">
                        <tr>
                            ...
                            <th class="px-3 py-2 border">Shop Name</th>
                            <th class="px-3 py-2 border">Product Name</th>
                            <th class="px-3 py-2 border">Category</th>
                            <th class="px-3 py-2 border">SubCategory</th>
                        ...
                        <tr>
                            ...
                            <td class="px-3 py-2">
                                <span class="fw-semibold text-primary">
                                    {{ $product->vendor->shop_name ?? $product->vendor->name ?? 'N/A' }}
                                </span>
                            </td>
                            <td class="px-3 py-2">{{ $product->name }}</td>
                            <td class="px-3 py-2">{{ $product->category->name ?? 'N/A' }}</td>
                            <td class="px-3 py-2">{{ $product->subCategory->name ?? 'N/A' }}</td>
            ...
            <div class="d-lg-none">
                ...
                <div class="border-bottom p-3">
                    ...
                    <div class="d-flex gap-2 mt-2">
                        <a href="{{ route('admin.products.edit', $product->id) }}" class="btn btn-primary btn-sm flex-fill">
                            <i class="fa fa-edit me-1"></i> Edit
                        </a>
                        <form id="delete-form-{{ $product->id }}" action="{{ route('admin.products.destroy', $product->id) }}" method="POST" class="flex-fill">
                            @csrf
                            @method('DELETE')
