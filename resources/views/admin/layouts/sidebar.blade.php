<nav id="sidebar">
    ...
    <style>
        ...
        #sidebar .nav-link.active i.menu-icon {
            color: #a78bfa;
        }
        ...
        #sidebar .collapse .nav-link i.menu-icon {
            width: 24px;
            font-size: 11.5px;
        }
...
{{-- Course (separate top-level dropdown) --}}
@canany(['create category','edit category','view category','delete category'])
@php
    $courseActive = request()->is('admin/course-categories*') || request()->is('admin/preparation-categories*');
@endphp
<li class="nav-item">
    <a class="nav-link d-flex align-items-center {{ $courseActive ? '' : 'collapsed' }}"
    data-bs-toggle="collapse" href="#courseMenu" role="button" aria-expanded="{{ $courseActive ? 'true' : 'false' }}">
        <i class="fas fa-graduation-cap menu-icon"></i>
        <span>Course</span>
        <i class="fas fa-chevron-right rotate"></i>
    </a>
    <div class="collapse {{ $courseActive ? 'show' : '' }}" id="courseMenu">
        <ul class="nav flex-column">
...
{{-- Ebook (separate top-level dropdown) --}}
@canany(['create category','edit category','view category','delete category','create product','edit product','view product','delete product'])
@php
    $ebookActive = request()->is('admin/ebook-categories*') || request()->is('admin/ebook-products*');
@endphp
<li class="nav-item">
    <a class="nav-link d-flex align-items-center {{ $ebookActive ? '' : 'collapsed' }}"
    data-bs-toggle="collapse" href="#ebookMenu" role="button" aria-expanded="{{ $ebookActive ? 'true' : 'false' }}">
        <i class="fas fa-book menu-icon"></i>
        <span>Ebook</span>
        <i class="fas fa-chevron-right rotate"></i>
    </a>
    <div class="collapse {{ $ebookActive ? 'show' : '' }}" id="ebookMenu">
        <ul class="nav flex-column">
            ...
            @canany(['create product','edit product','view product','delete product'])
            <li><a class="nav-link {{ request()->is('admin/ebook-products*') && !request()->is('admin/ebook-products/create') ? 'active' : '' }}" href="{{ route('admin.ebook-products.index') }}">
                <i class="fas fa-th-large menu-icon"></i> All Ebook Products
            </a></li>
            <li><a class="nav-link {{ request()->is('admin/ebook-products/create') ? 'active' : '' }}" href="{{ route('admin.ebook-products.create') }}">
                <i class="fas fa-plus menu-icon"></i> Add Ebook Product
            </a></li>
            @endcanany
...
@endcanany
        ...
        <li class="nav-item">
            ...
            <div class="collapse {{ $customerActive ? 'show' : '' }}" id="customerMenu">
                <ul class="nav flex-column">
                    <li><a class="nav-link {{ request()->is('admin/customers') || request()->is('admin/customers?*') ? 'active' : '' }}"
                            href="{{ route('admin.customers.index') }}">
                            <i class="fas fa-users menu-icon"></i> All Customers
                        </a></li>
                    <li><a class="nav-link {{ request()->is('admin/customers/create') ? 'active' : '' }}"
    ...
    @php $websiteActive = request()->is('admin/smtp*') || request()->is('admin/courier*') || request()->is('admin/marketing*') || request()->is('admin/bannars*') || request()->is('admin/settings*') || request()->is('admin/customer-review*')|| request()->is('admin/showroom*') || request()->is('admin/contacts*') || request()->is('admin/how-to-buy*') || request()->routeIs('admin.about.*') || request()->routeIs('admin.ebook-library.*') || request()->routeIs('admin.faqs.*'); @endphp
    <li class="nav-item">
        ...
        <div class="collapse {{ $websiteActive ? 'show' : '' }}" id="websiteMenu">
            <ul class="nav flex-column">
                ...
                @canany(['create courier', 'view courier'])
                    <li><a class="nav-link {{ request()->is('admin/courier*') ? 'active' : '' }}"
                            href="{{ route('admin.courier.setup') }}">
                            <i class="fas fa-shipping-fast menu-icon"></i> Courier Setup
                        </a></li>
                @endcanany
                <li><a class="nav-link {{ request()->is('admin/coupons*') ? 'active' : '' }}"
                        href="{{ route('admin.coupons.index') }}">
                        <i class="fas fa-ticket-alt menu-icon"></i> Coupons
                    </a></li>
                @canany(['create marketing', 'view marketing'])
                    <li><a class="nav-link {{ request()->is('admin/marketing*') ? 'active' : '' }}"
                            href="{{ route('admin.marketing.setup') }}">
                            <i class="fas fa-bullhorn menu-icon"></i> Marketing
                        </a></li>
                @endcanany
                @canany(['create banner', 'view banner'])
                    <li><a class="nav-link {{ request()->is('admin/bannars*') ? 'active' : '' }}"
                            href="{{ route('admin.bannars.index') }}">
                            <i class="fas fa-images menu-icon"></i> Banners
                        </a></li>
                @endcanany
                @canany(['create banner', 'view banner'])
                    <li><a class="nav-link {{ request()->is('admin/customer-review*') ? 'active' : '' }}"
                            href="{{ route('admin.customer-review.index') }}">
                            <i class="fas fa-star menu-icon"></i> Customer Review
                        </a></li>
                @endcanany
                <li><a class="nav-link {{ request()->is('admin/contacts*') ? 'active' : '' }}"
                        href="{{ route('admin.contacts.index') }}">
                        <i class="fas fa-address-book menu-icon"></i> Contact
                    </a></li>
                <li><a class="nav-link {{ request()->is('admin/how-to-buy*') ? 'active' : '' }}"
                        href="{{ route('admin.how-to-buy.index') }}">
                ...
                {{-- ── About, Ebook Library, FAQs ── --}}
                <li><a href="{{ route('admin.about.index') }}"
                        class="nav-link {{ request()->routeIs('admin.about.*') ? 'active' : '' }}">
                        <i class="fas fa-info-circle menu-icon"></i> About
                    </a></li>

                <li><a href="{{ route('admin.ebook-library.index') }}"
                        class="nav-link {{ request()->routeIs('admin.ebook-library.*') ? 'active' : '' }}">
                        <i class="fas fa-book menu-icon"></i> Ebook Library
                    </a></li>

                <li><a href="{{ route('admin.faqs.index') }}"
                        class="nav-link {{ request()->routeIs('admin.faqs.*') ? 'active' : '' }}">
                        <i class="fas fa-question-circle menu-icon"></i> FAQs
                    </a></li>
            </ul>
        </div>
    </li>
@endcanany
        ...
        <li class="nav-item">
            ...
            <div class="collapse {{ $landingActive ? 'show' : '' }}" id="landingPageMenu">
                <ul class="nav flex-column">
                    <li><a class="nav-link {{ request()->is('admin/legal-policy*') ? 'active' : '' }}"
                            href="{{ route('admin.legal-policy.index') }}">
                            <i class="fas fa-file-contract menu-icon"></i> Legal & Policy
                        </a></li>
                    <li><a class="nav-link {{ request()->is('admin/navigation*') ? 'active' : '' }}"
        ...
        @canany(['create report', 'edit report', 'view report', 'delete report'])
            ...
            <li class="nav-item">
                ...
                <div class="collapse {{ $reportsActive ? 'show' : '' }}" id="reportsMenu">
                    <ul class="nav flex-column">
                        <li><a class="nav-link {{ request()->is('admin/stock-report*') ? 'active' : '' }}"
                                href="{{ route('admin.stock_report') }}">
                                <i class="fas fa-boxes menu-icon"></i> Stock Report
                            </a></li>
                        <li><a class="nav-link {{ request()->is('admin/order-report*') ? 'active' : '' }}"
        ...
        @canany(['create user', 'edit user', 'view user', 'delete user', 'create role', 'edit role', 'view role',
            'delete role'])
                ...
                <div class="collapse {{ $userActive ? 'show' : '' }}" id="userManagementMenu">
                    <ul class="nav flex-column">
                        ...
                        @canany(['create role', 'view role'])
                            <li><a class="nav-link {{ request()->is('admin/roles*') ? 'active' : '' }}"
    ...
    <div class="sidebar-footer">
        ...
        <a href="{{ route('admin.logout') }}"
            onclick="event.preventDefault(); document.getElementById('logout-form').submit();" class="sf-logout"
            title="Logout">
